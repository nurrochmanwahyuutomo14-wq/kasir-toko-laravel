<div class="space-y-6 p-6 bg-gray-50 min-h-screen">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <button wire:click="setFilter('today')"
                    ...
                    {{ $tgl_awal == date('Y-m-d') && $tgl_akhir == date('Y-m-d') ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                    Hari Ini
                </button>
                <button wire:click="setFilter('week')"
                    class="px-4 py-2 rounded-xl text-xs font-bold transition-all
                    {{ $tgl_awal == date('Y-m-d', strtotime('-7 days')) ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                    1 Minggu
                </button>
                <button wire:click="setFilter('month')"
                    class="px-4 py-2 rounded-xl text-xs font-bold transition-all
                    {{ $tgl_awal == date('Y-m-d', strtotime('-30 days')) ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                    1 Bulan
                </button>
                ...
                <div class="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-xl border border-gray-200">
                    <input type="date" wire:model.live="tgl_awal" class="bg-transparent text-xs font-semibold outline-none text-gray-700">
                    <span class="text-gray-300 text-xs">—</span>
                    <input type="date" wire:model.live="tgl_akhir" class="bg-transparent text-xs font-semibold outline-none text-gray-700">
                </div>

                <a href="/kasir" class="px-4 py-2 rounded-xl text-xs font-bold bg-blue-50 text-blue-600 hover:bg-blue-100 transition-all border border-blue-100">
                    ← Kasir
                </a>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 border-l-4 border-l-green-500">
            ...
            <h2 class="text-2xl font-black text-gray-800">Rp {{ number_format($todaySales, 0, ',', '.') }}</h2>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 border-l-4 border-l-blue-500">
            ...
            <h2 class="text-2xl font-black text-gray-800">{{ $todayTransactions }} <span class="text-sm font-normal text-gray-400">Nota</span></h2>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span class="text-xs font-bold px-2.5 py-1 rounded-full {{ $trx->payment_method == 'Cash' ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600' }}">
                        ...
                        <td class="px-6 py-4 text-right">
                            <span class="font-bold text-gray-800 text-sm">Rp {{ number_format($trx->total_price, 0, ',', '.') }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <a href="{{ route('print.struk', $trx->id) }}" target="_blank"
                                class="inline-flex items-center gap-1 text-xs font-bold px-3 py-1.5 rounded-lg bg-gray-100 text-gray-600 hover:bg-blue-100 hover:text-blue-600 transition-all">
                                📄 Struk
                            </a>
                        </td>
                    </tr>
